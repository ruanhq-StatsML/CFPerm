...
\paragraph{Algorithm and opportunities.}
At online step \(t\), AGOD treats the modality-specific gap as a clever covariate rather than a post-hoc diagnostic: given reference batch \(\mathcal{D}_0\) and stream batch \(\mathcal{D}_t\), it estimates \(g_m^{(t)}=\mathrm{Normalize}\bigl(\mathrm{AUC}_m^{(t)}\cdot\mathrm{VIMP}_m^{(t)}+\gamma\cdot\mathrm{PO\text{-}risk}_m^{(t)}\bigr)\) for \(m\in\{\mathrm{audio},\mathrm{image},\mathrm{text}\}\) via an RF domain classifier and a CFPerm meta-learner (with RF-Domain-only fallback when overlap \(<0.3\)), maps \(\boldsymbol{\alpha}^{(t)}=\mathrm{Softmax}(\mathbf{g}^{(t)}/\tau)\) through a gated EMA smoother, and updates the student by \(\mathcal{L}^{(t)}=\mathcal{L}_{\mathrm{global}}^{(t)}+\sum_m\alpha_m^{(t)}\mathcal{L}_m^{(t)}+\lambda\Omega(\boldsymbol{\alpha}^{(t)})\). The control signal already routes capacity toward concept drift, but task gains remain small; the natural next steps are (i)~replacing the linear teacher/student with frozen CLIP/CLAP/EmbeddingGemma on true ChronoBerg windows, (ii)~allocating \emph{update budget} (steps or tokens) by \(\boldsymbol{\alpha}\) with a global-loss floor so stable modalities are not starved, (iii)~calibrating \(\gamma,\tau,\lambda\) and using residualized CATE heterogeneity so strong covariate shift does not leak PO-risk across views, and (iv)~extending alignment from modality-level MSE to slice/patch-token and sample-wise weights, evaluated with multi-seed retrieval, forgetting curves, and FLOPs when only one modality drifts.
...
\section{Multi-suite evidence and LLM inference boosting}
\label{sec:boost}

We re-run AGOD on three shift suites (ChronoBerg calendar, newsgroups topic windows, Amazon-style review polarity) and add three compute policies: Sparse-AGOD (B4) drops low-\(\alpha\) modalities, Budget-AGOD (B5) spends a \(1/|\mathcal{M}|\) step budget, and Cascade-AGOD (B6) skips distillation when domain AUC is small.

\input{results/tab_agod_suites.tex}

The same clever covariate can gate \emph{decode} compute. A compressed student always drafts; the teacher is invoked only when prompt novelty vs.\ a frozen reference pool (MSG) is high.

\begin{center}
\includegraphics[width=0.95\linewidth]{results/llm_boost_diagram.png}
\end{center}

\input{results/tab_llm_boost.tex}
