...
from agod.po_refit import make_batch_stream, run_adaptive_stream, run_refit_stream, run_uniform_on_same_rows
...
def write_md(summary, gate, path):
    ...
    lines += [
        ...
        "  gated fire-rate should stay low. Always-on √PO is worse.",
        "- **concept / mixed**: a clear hop → drop the old batch (hop / adaptive).",
        "  Pair-uniform mixes pre- and post-shift and is worst on concept.",
        "- pair = train on last two; hop = new batch only;",
        "  adaptive = pair+uniform when quiet, hop+√PO when the hop contrast fires.",
        "",
    ]
    path = Path(path)
...
def write_tex(summary, gate, path):
    lines = [
        ...
        r"Scene & uniform (pair) & gated pair & always PO & adaptive & fire \\",
        r"\midrule",
    ]
    for scene, cell in summary.items():
        m = cell["methods"]
