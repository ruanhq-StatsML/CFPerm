#Evaluation of power for the Model B from a benchmark method 
...
for(j in 1:4){
  ...
  for(i in 1:B){
    ...
    df_train1 <- model_B_DGP(n, alpha = 0, train = TRUE, beta = beta1)
    df_train2 <- model_B_DGP(n, alpha = 0, train = TRUE, beta = beta1)
    df_test1 <- model_B_DGP(n, alpha = 0, train = FALSE, beta = beta1)
    df_test2 <- model_B_DGP(n, alpha = 0.5, train = FALSE, beta = beta1)
    ####
  ...
  write.csv(power_df, "power_df_modelB_CFPerm12.csv")
}
