...
If continuous inference includes a test-time LoRA or gated residual, the update \emph{support} should be the vision tower and fusion, then audio, not the language tower against $W$.
The \emph{step} has the opposite sign on covariate intensity: large $c_m^{\mathrm{cov}}$ lowers $\eta_m$; large concept drift $\delta_m^{\mathrm{concept}}$ raises it.
Clip-level text is both-quiet, so $\eta_{\mathrm{t}}\approx 0$.
