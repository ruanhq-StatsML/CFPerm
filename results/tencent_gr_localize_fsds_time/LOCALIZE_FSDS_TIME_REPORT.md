...
3. User-split on W1；localization shares / subset 只来自 **W1-train**（三段启发式 aggregate through）
4. FSDS select+train 只在 W1-train；W2 仅作 temporal holdout
...
- CTR-style edges: require `n_exp>0`; drop click/cnv/ctr features from X
- localization shares fit on **W1-train users only**, frozen onto W2
- SelectKBest never sees W2 labels
