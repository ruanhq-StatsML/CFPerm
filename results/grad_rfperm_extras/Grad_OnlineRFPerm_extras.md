...
Definition: **FAR = (# reject batches) / (# observation batches)** on the post-burn (optionally post-grace) window.
...
- **Null FAR:** $\mathrm{FAR}=n_{\mathrm{alarm}}/n_{\mathrm{batch}}$ (alarms over observation batches). Grad FAR ≈ MSE FAR (~0.20–0.23); grace mainly delays first reject, mild FAR drop.
